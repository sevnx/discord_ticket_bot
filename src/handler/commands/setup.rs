...
pub async fn setup(ctx: Context<'_>) -> Result<(), Error> {
    ...
    if is_server_setup(&mut pool, guild_id).await? {
    ...
    guild_info.unclaimed_category(
        create_server_category(&ctx, guild_id, "Unclaimed Tickets")
            .await?
            .id,
    );
    guild_info.claimed_category(
        create_server_category(&ctx, guild_id, "Claimed Tickets")
            .await?
            .id,
    );
    ...
    // Roles
    guild_info
        .helper_role(get_new_or_existing_role(&ctx, guild_id, "Helper Role", "Helper").await?);

    guild_info.moderator_role(
        get_new_or_existing_role(&ctx, guild_id, "Moderator Role", "Moderator").await?,
    );
...
impl ServerSetupData {
    ...
    async fn setup_reaction_message(&self, ctx: &Context<'_>) -> Result<MessageId, Error> {
        let embed = CreateEmbed::default_bot_embed(self.guild.to_partial_guild(ctx.http()).await?)
            .title("Open a ticket")
